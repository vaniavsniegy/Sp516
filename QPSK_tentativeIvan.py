...
#---------- Configuration ------------#
#-------------------------------------#
f = open("video_binaire.bin", "rb")
bin = f.read()
struct.unpack("i" * ((len(bin) -24) // 4), bin[20:-4])
...
# print(bits_str)
# inputBites = np.random.randn(20,1) > 0 ; print(inputBites)
# print(type(inputBites))

bits = np.array([[int(c)] for c in bits_str])

Nbits = len(bits)
# print(len(bits))

f0 = 1800                   # carrier Frequency
Ns = fs/baud                # number of Samples per Symbol
...
if(len(inputBits)%2 == 0):
    ...
    I_signal = (np.tile(int.from_bytes(I_bits)*2-1,(1,2*Ns))).ravel()


    Q_signal = (np.tile(int.from_bytes(Q_bits)*2-1,(1,2*Ns)) ).ravel()
